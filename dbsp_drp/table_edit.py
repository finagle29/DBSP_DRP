...
class TableModel(QtCore.QAbstractTableModel):
    def __init__(self, data: Table, del_files: list, instrument: Instrument = None):
        ...
        self._data = data
        self._cols = (
            instrument.table_edit_columns
            if instrument.table_edit_columns is not None
            else data.colnames
        )
    ...
    def _update_fits(self) -> None:
        ...
        for fname in self._modified_files:
            ...
            with fits.open(path, mode='update') as hdul:
                for col in self._instrument.table_edit_editable_columns:
                    if not maskrow[col]:
                        update_header(hdul[0].header,
                            self._instrument.column_to_header_kw(col),
                            self._instrument.table_edit_hdu_formatter(col, row[col][0]),
                            f' Modified {now_str}')
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, data: Table, del_files: list = None, instrument: Instrument = None):
        super().__init__()

        self.table = TableView(TableModel(data, del_files, instrument))
...
def main(table, del_files: List[str], instrument: Instrument):
    if not QtWidgets.QApplication.instance():
        app = QtWidgets.QApplication([])
    else:
        app = QtWidgets.QApplication.instance()
    window = MainWindow(table, del_files, instrument)
    window.show()
    app.exec_()
